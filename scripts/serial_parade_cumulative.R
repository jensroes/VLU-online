...
# Model formulas
f_1 <- bf(resp_given ~  1 )
f_2 <- bf(resp_given ~  1 + cs(resp_expec))
f_3 <- bf(resp_given ~  1 + FA)
f_4 <- bf(resp_given ~  1 + resp_expec + FA)
f_5 <- bf(resp_given ~  1 + resp_expec * FA)
...
for(i in fs){
  ...
  assign(output_name,
    fit_brms_cm(formula = eval(as.name(i)), data = data)
  )
}
...
file_out <- "stanout/loo_results_serial.csv"
