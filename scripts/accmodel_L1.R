...
d %<>% filter(L1eng == 1) %>% dplyr::select(-L1eng);d
...
file_out <- "stanout/loo_results_accmodel_L1.csv"
...
saveRDS(modelfit_6,
        file="stanout/accmodel_L1.rda",
        compress="xz")

samps <- posterior_samples(modelfit_6, "^b_COND") %>%
  as_tibble() 

write_csv(samps, "stanout/posterior_accmodel_L1.csv")
