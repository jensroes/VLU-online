...
for(i in fs){
  ...
  assign(output_name,
    fit_brms_sratio(formula = eval(as.name(i)), data = data)
  )
}
...
saveRDS(modelfit_2,
        file="stanout/sequential_sratio.rda",
        compress="xz")

saveRDS(modelfit_3,
        file="stanout/sequential_sratio_cs.rda",
        compress="xz")
...
conditional_effects(modelfit_4, 
                      robust = TRUE, # median instead of mean
                      effects = "TP", 
                      conditions = data.frame(responses = c(3,7),
                                              corr_response = "incorrect"))
  
conditional_effects(modelfit_4, 
                      robust = TRUE, # median instead of mean
                      effects = "corr_response", 
                      conditions = data.frame(responses = c(3,7),
                                              TP = "present")) # correct response
  
conditional_effects(modelfit_4, 
                      robust = TRUE, # median instead of mean
                      effects = "responses", 
                      conditions = data.frame(
                        corr_response = "incorrect",# c("incorrect"),
                        TP = "present"# c("present", "absent")
                      ))
