...
m_serial <- readRDS(file="stanout/sequential_countmodel_serial.rda")
m_seq <- readRDS(file="stanout/sequential_countmodel.rda")
...
plot_resp <- ggplot(data= ps, aes(x = responses, y = estimate__, 
  ...
  labs(x = "Lineup ID",
       ...
       title = "Sequential process model") + theme(legend.key.height = unit(1, "cm")) ; plot_resp
